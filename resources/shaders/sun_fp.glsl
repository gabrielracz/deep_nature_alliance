...
vec4 lighting(vec4 pixel, int i, vec3 lv, vec3 n) {
    ...
    return diffuse_strength*diffuse*lights[i].color*pixel + (lights[i].ambient_strength + 1.0) * lights[i].ambient_color*pixel + 0.9f * spec*lights[i].color;
}
...
void main() 
{
    ...
    vec3 mixedcol = mix(pixel.xyz, ridgecol, intensity);
